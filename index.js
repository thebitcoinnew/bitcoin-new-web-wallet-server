...
/** Configuration **/
  const btcnewNodeUrl = process.env.BTCNEW_NODE_URL || `http://[::1]:9076`; // Bitcoin New node RPC url
  const btcnewWorkNodeUrl = process.env.BTCNEW_WORK_NODE_URL || `http://[::1]:9076`; // Bitcoin New work node RPC url
...
const redisCacheUrl = process.env.REDIS_HOST || `127.0.0.1`; // Url to the redis server (If used)
...
// Allow certain requests to the Bitcoin New RPC and cache work requests
app.post('/api/node-api', async (req, res) => {
  ...
  if ((workRequest || representativeRequest) && !nodeOverride) {
    nodeUrl = btcnewWorkNodeUrl;
  }

  // Determine if we use DPoW instead of proxying to the btcnew node
  ...
  // Send the request to the Bitcoin New node and return the response
